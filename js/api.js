...
export async function fetchUsageStats() {
  ...
  if (error) { console.error(error); return { total_bytes: 0, total_count: 0 }; }
  return data?.[0] ?? { total_bytes: 0, total_count: 0 };
}

// Paginated + server-side filtered/sorted list. Never loads the whole table.
export async function fetchFilesPage({ category, search, sortField, sortDir, offset, limit = PAGE_SIZE }) {
...
export async function uploadFile(file, userId) {
  ...
  const { data, error: dbErr } = await sb.from('vault_files').insert({
    user_id: userId,
    file_name: file.name,
    storage_path: path,
