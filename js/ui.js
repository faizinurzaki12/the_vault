...
const thumbObserver = new IntersectionObserver(
  (entries) => {
    for (const entry of entries) {
      ...
      getSignedUrl(img.dataset.lazy).then((url) => {
        if (url) img.src = url;
      });
    }
  },
...
export function renderFolderTiles({ folders, showBack, onOpenFolder, onBack, onNewFolder }) {
  ...
  if (showBack) {
    ...
    for (const f of folders) {
      ...
      card.innerHTML = `<div class="card-thumb">📁</div><div class="card-body"><div class="card-name" title="${f.name}">${f.name}</div></div>`;
      card.addEventListener("click", () => onOpenFolder(f));
...
export function appendFileCards(rows, onOpen) {
  ...
  for (const f of rows) {
    const card = document.createElement("div");
    card.className = "card";
    const thumbInner = f.category === "image" ? `<img data-lazy="${f.storage_path}" alt="">` : iconFor(f.category);
    card.innerHTML = `
...
async function renderZipContents(url, visual) {
  try {
    ...
    if (!entries.length) {
      visual.innerHTML = '<span class="zip-empty">Empty archive.</span>';
      return;
    }
    visual.innerHTML = `<ul class="zip-list">${entries
      .map(
        (e) => `
      <li class="zip-row"><span>${e.name}</span><span>${formatBytes(e._data?.uncompressedSize ?? 0)}</span></li>`,
      )
      .join("")}</ul>`;
  } catch (err) {
    console.error(err);
